...
from typing import TypeVar, Type, Optional, Tuple, Callable, List
...
def previous_epoch(years: int = 0, days: int = 0, hours: int = 0, minutes: int = 0, seconds: int = 0) -> float:
    total_loss = (((years * 365.0 + days) * 24.0 + hours) * 60 + minutes) * 60.0 + seconds
...
def test_performance(func):
    ...
    def inner(*args, **kwargs):
        ...
        _logger.info(f"Function {func.__name__} took {time.perf_counter() - timeStart} seconds to run.")
        return return_val
    return inner
...
def thread_over_data(data: List[any], data_cb: Callable, threads: int = 4) -> None:
    ...
    thread_pool = [Thread(target=do_thread, args=(split_data[i],)) for i in range(num_threads)]
